...
# 3.- Load database -------------------------------------------------------
BorBryoRare4 <- read.csv("SPMO_BryoAbundance.csv")
str(BorBryoRare4)

# Delete first column ($ X) created when the data is loaded.
BorBryoRare4 <- BorBryoRare4[,-1] 
...
Value <- rep(1, times = 268) # times as many rows you have
...
View(BryRarForms2)
sum(BryRarForms1$Value) # Total species in the SPMO domain
...
pdf(file = "Fig.2a_SPMO a) SPMO bioclimatic domain.pdf",   
    ...
    ggplot(data=BryRarForms2, 
       ...
       labs(title="a) SPMO bioclimatic domain (267 species)",
